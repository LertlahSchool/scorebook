      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          { label: isAdmin ? 'นักเรียนทั้งหมด' : 'วิชาที่สอน',
            value: isAdmin ? studentCount ?? 0 : mySubjects?.length ?? 0,
            color: '#4F46E5' },
          { label: isAdmin ? 'ครูในระบบ' : 'ห้องเรียน',
            value: isAdmin ? teacherCount ?? 0 : new Set(mySubjects?.map(s => s.class_id)).size,
            color: '#F59E0B' },
          { label: 'รายการคะแนนที่บันทึก',
            value: scoreCount ?? 0,
            color: '#059669' },
        ].map((stat, i) => (
        ...
        <div className="card">
          ...
          {mySubjects && mySubjects.length > 0 ? (
            ...
            <div className="text-center py-10 text-slate-400">
              ...
              <div className="text-xs mt-1">กรุณาติดต่อ Admin</div>
            </div>
          )}
        </div>
      )}
